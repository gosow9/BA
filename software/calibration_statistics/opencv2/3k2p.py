...
f = open('3k2p_params.txt', 'w')

for i in range(n):
    objp = np.load('checkerboards/objp{:}.npy'.format(i))
    imgp = np.load('checkerboards/imgp{:}.npy'.format(i))

    # select model with 3 ks and 2 ps
    ret, mtx, dist, rvecs, tvecs, newobjp, stdin, stdex, pve, stdnewobjp = cv2.calibrateCameraROExtended(objp, imgp, (3280, 2464), 1, None, None)
    ...
    dist_x = fx*full_dist_model_x(k1, k2, k3, p1, p2, x/fx, y/fx, new_r)
    dist_y = fy*full_dist_model_y(k1, k2, k3, p1, p2, x/fx, y/fy, new_r)
