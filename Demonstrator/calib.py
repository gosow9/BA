...
images = glob.glob('calibration_files/low_dist/*.png')
...
for fname in images:
    ...
    if ret == True:
        ...
        for c in corners:
            cv2.drawMarker(img, (c[0][0], c[0][1]), (0, 0, 255), markerSize=4)
            
            cv2.namedWindow('img', cv2.WINDOW_NORMAL)
            cv2.resizeWindow('img', 816, 462)
            cv2.imshow('img',img)
    
            cv2.waitKey(500)
    
    else:
        rejected_im.append(fname)
...
# calibrate camnera
ret, mtx, dist, rvecs, tvecs, newobjp, stdin, stdex, pve, stdnewobjp = cv2.calibrateCameraROExtended(objpoints, imgpoints, (3264, 1848), 1, None, None, flags=flags, criteria=criteria)

# store result to files
np.savetxt('calibration_files/low_dist/mtx_lowdist.txt', mtx)
np.savetxt('calibration_files/low_dist/dist_lowdist.txt', dist)

with open('calibration_files/low_dist/params_lowdist.txt', 'w') as f:
